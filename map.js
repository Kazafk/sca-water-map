    ...
    if (action === 'switch-tab' && activeCommune) {
      ...
      if (tab === 'data') {
        ...
        _fetchRawData(insee).then(rawData => {
          if (activeTab === 'data' && activeCommune?.insee === insee)
            updatePanel(activeCommune, generatedAt, totalScored, { showBottled, activeTab: 'data', rawData, selectedCode: selectedSandreCode });
        });
      } else {
